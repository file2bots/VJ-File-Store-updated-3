...
async def handle_message(client, message):
    try:
        ...
        await delete_previous_reply(chat_id)  # Ensure old replies are deleted
        ...
        if chat_id in user_states:
            ...
            if current_state == "awaiting_num_files":
                ...
                if message.media:
                    ...
                    forwarded_message = await message.copy(chat_id=DIRECT_GEN_DB)  # Copy instead of forward

                    if not forwarded_message:
                        await message.reply("Error: Message forwarding failed.")
                        return

                    file_id = forwarded_message.message_id  # Correct file_id assignment
                    log_msg = await message.copy(chat_id=DIRECT_GEN_DB)

                    if not log_msg:
                        await message.reply("Error: Failed to log the message.")
                        return

                    stream_link = await gen_link(log_msg)  # Ensure gen_link is valid
                    size = get_size(getattr(message, file_type.value).file_size)
                    await message.delete()

                    # Append details to user state
                    ...
                    if num_files_left > 0:
                        reply_message = await message.reply(f"**⏩ ғᴏʀᴡᴀʀᴅ ᴛʜᴇ ɴᴏ: {files_received + 1} ғɪʟᴇ**")
                        user_states[chat_id]["last_reply"] = reply_message                     
                    else:
                ...
                await message.delete()
                del user_states[chat_id]  # Clear user state after completion
